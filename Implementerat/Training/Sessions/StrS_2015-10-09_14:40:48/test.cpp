...
vector<int> prefix( int m, const char P[] ){
...
int stringSearch( const char T[], int n, const char P[], int m ) {
	...
	int p = 0;
	vector<int> pi = prefix( m, P );

	// cout << T << endl;
	// cout << P << endl;
	// for (std::vector<int>::iterator i = pi.begin(); i != pi.end(); ++i)
	// {
	// 	cout << *i << endl;
	// }
	...
	for (int i = 0; i < n; ++i)
	{
		// cout << p << " " << i << " " << P[p] << " " <<  T[i] << endl;
		while ( p>0 and T[i]!=P[p] ){
			p = pi.at(p-1);
		}
