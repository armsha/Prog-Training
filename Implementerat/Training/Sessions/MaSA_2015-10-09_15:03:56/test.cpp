...
AnswerTriplet maximumSubarray( int A[], int n ){
	...
	for (int i = 0; i < n; ++i)
	{
		if ( currsum > 0 and currsum + A[i] > 0 )
		{
			currsum = currsum + A[i];
			cq = i;
		} else {
		...
		if ( ( currsum > msum  ) or ( currsum == msum and cq-cp < mq-mp )  )
		{
			msum = currsum;
			mq = cq;
			mp = cp;
	...
	if (msum <= 0)
	{
		...
		return AnswerTriplet( mp, mq, msum );
...
void testLargeArray(){
	...
	// cout << ansV.p << " " << ansV.q << " " << ansV.sum << endl;
	// cout << ansC.p << " " << ansC.q << " " << ansC.sum << endl;
