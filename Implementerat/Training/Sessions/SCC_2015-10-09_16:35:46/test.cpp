...
void visit( vertex *v, int *i, vector< vector<int> > &c, vector<vertex *> &s ){
	...
	*i = *i+1;
	...
	for (std::vector<edge>::iterator e = v->neighbours.begin(); e != v->neighbours.end(); ++e)
	{
		...
		{
			visit( u, i, c, s );
			v->cansee = min( v->cansee, u->cansee );
		}else if ( u->colour == GRAY ){
			v->cansee = min( v->cansee, u->index );
		}
	}
	...
	if ( v->index == v->cansee )
	{
		...
		vertex *top = s.back();
		s.pop_back();
		component.push_back(top->label);
		top->colour = BLACK;

		while ( top->label != v->label ){
