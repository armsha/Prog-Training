...
point findLow( point S[], int n ){
	...
	{
		cc = S[i];
		if ( ( lowest.second >= cc.second + 0.00000001 )
			or ( ( lowest.first < cc.first ) and ( lowest.second >= cc.second - 0.00000001 ) and ( lowest.second <= cc.second + 0.00000001 ) ) )
		{
			lowest = cc;
		}
	}
...
point findHigh( point S[], int n ){
	...
	{
		cc = S[i];
		if ( ( highest.second <= cc.second - 0.00000001 )
			or ( ( highest.first > cc.first ) and ( highest.second >= cc.second - 0.00000001 ) and ( highest.second <= cc.second + 0.00000001 ) ) )
		{
			highest = cc;
		}
	}
...
double getAngR( point p, point n ){

	double dx = n.first - p.first;
	double dy = n.second - p.second;

	return atan2( dy,dx );
}

double getAngL( point p, point n ){
...
vector< point > convex_hull( point S[], int n ){
	...
	point low = findLow(S, n);
	point high = findHigh(S, n);
	point next, potential, best, p;
	double bestAng, ang;
	...
	while ( p != high ){
		...
		// cout << endl << "a " << p.first << "," << p.second << " " << high.first << "," << high.second << endl << endl;
		...
		for (int i = 0; i < n; ++i)
		{
			...
			if ( p.second <= next.second + 0.0000001 )
			{
				ang = getAngR( p, next );
				// cout << ang << " " << bestAng << endl;
				if ( ang <= bestAng )
				{
					bestAng = ang;
					best = next;
		...
		if ( bestAng >= getAngR( p, high ) - 0.00000001 and bestAng <= getAngR(p, high))
		{
			p = best;
			continue;
		}
	...
	while ( p != high ){
		...
		for (int i = 0; i < n; ++i)
		{
			...
			if ( p.second <= next.second + 1000000 )
			{
				ang = getAngL( p, next );
				if ( ang <= bestAng )
				{
		...
		if ( bestAng >= getAngL( p, high ) - 0.00000001 and bestAng <= getAngL(p, high))
		{
			p = best;
			continue;
		}
 ...
 *	TIME: 60
 *	COMPLEXITY: jarvise giftwrapping O(n*h)
 */
