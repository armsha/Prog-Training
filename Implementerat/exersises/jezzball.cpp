...
public:
	...
	double intersects( double t, int Px, int Py, int h, int w, int rw ) {
		...
		cout << endX << " " << endY << " " <<  currX << " " << currY << " " << intersectX << " " << intersectY << endl;
		...
		if ( intersectX==-1 )
		{
				 ...
				 ( currX > Px and endX < Px ) )
			{
				...
				if (intersectY1 >= Py)
				{
					...
					{
						return ((Py+rw*intersectTime1)-intersectY1)/rw;
					}
				} else {
					if ( Py-rw*intersectTime1 < intersectY1)
					{
						return (intersectY1-(Py-rw*intersectTime1))/rw;
					}
				}
			}
		} else {
				 ...
				 ( currX > Px and endX < Px ) )
			{
				...
				if ( endX > Px ){
					...
					if (intersectY2 >= Py)
					{
						...
						{
							return ((Py+rw*intersectTime2)-intersectY2)/rw;
						}
					} else {
						if ( Py- rw*intersectTime2 < intersectY2)
						{
							return (intersectY2-(Py-rw*intersectTime2))/rw;
						}
					}
				}
...
int main(int argc, char const *argv[])
{
	...
	while (true) {
		...
		{
			cin >> xa >> ya >> vax >> vay;
			atoms.push_back( atom( xa, ya, vax, vay ) );
			atoms2.push_back( atom( h-ya, xa, -vay, vax ) );
		}

		cout << "SVAR NORMAL" << solve( atoms, x, y, h, w, rv ) << endl;
		cout << "SVAR ROTERA" << solve( atoms2, h-y, x, w, h, rv ) << endl;
